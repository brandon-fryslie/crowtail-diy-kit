#!/usr/bin/env python3
#
# Modified by Brandon Fryslie to use Python3
# Mainly just fixed some types regarding str vs bytes
#
# ESP8266 luatool
...
class AbstractTransport:
    ...
    def writeln(self, data: str, check: int=1):
        raise NotImplementedError('Function not implemented')
    ...
    def performcheck(self, expected: str):
        ...
        i = -1
        while char != chr(62):  # '>'
            char = self.read(1).decode('utf-8')
            if char == '':
                raise Exception('No proper answer from MCU')
            if char == chr(13) or char == chr(10):  # LF or CR
                if line != '':
                    line = line.strip()
                    if f'{line}\r' == expected and not args.bar:
                        sys.stdout.write(" -> ok")
...
class SerialTransport(AbstractTransport):
    def __init__(self, port: str, baud: int, delay: float):
        self.port = port
        self.baud = baud
        self.serial = None
        self.delay = delay
    ...
    def writeln(self, data: Union[str, bytes], check: int=1):
        if self.serial.inWaiting() > 0:
            self.serial.flushInput()
        if len(data) > 0 and not args.bar:
            sys.stdout.write("\r\n->")

            # handle bytes separately from str
            if type(data) == str:
                split_data: List[bytes] = data.split("\r")
                split_item = split_data[0]
            elif type(data) == bytes:
                split_data: List[bytes] = data.split("\r".encode("utf-8"))
                split_item = split_data[0].decode('utf-8')
            else:
                raise RuntimeError(f"Error: Invalid type {type(data)} for incoming data")

            sys.stdout.write(split_item)

        # serial always wants bytes
        if type(data) == str:
        ...
        sleep(self.delay)
        if check > 0:
            self.performcheck(data.decode('utf-8'))
        elif not args.bar:
...
class TcpSocketTransport(AbstractTransport):
    ...
    def writeln(self, data, check: int=1):
        if len(data) > 0 and not args.bar:
            sys.stdout.write("\r\n->")
            sys.stdout.write(data.split("\r")[0].encode("utf-8"))
        # this is probably broken due to str vs bytes
...
def decidetransport(cliargs: argparse.Namespace):
    if cliargs.ip:
...
if __name__ == '__main__':
    ...
    if args.append==False:
        ...
        transport.writeln(f'file.open("{args.dest}", "w")\r'.encode("utf-8"))
        transport.writeln("file.close()\r".encode("utf-8"))
        transport.writeln(f'file.remove("{args.dest}")\r'.encode("utf-8"))
    else:
        if args.verbose:
